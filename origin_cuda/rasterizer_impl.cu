    ...
    bool debug)
{
    ...
    CHECK_CUDA(cub::DeviceScan::InclusiveSum(
                   ...
                   P),
               debug)
    u_int32_t num_rendered1;
    CHECK_CUDA(cudaMemcpy(&num_rendered1, geomState.tiles_touched, sizeof(u_int32_t), cudaMemcpyDeviceToHost),
               true);
    int rr = 0;

    u_int32_t num_rendered2;
    CHECK_CUDA(cudaMemcpy(&num_rendered2, geomState.tiles_touched + 1, sizeof(u_int32_t), cudaMemcpyDeviceToHost),
               true);
    ...
    int num_rendered;
    CHECK_CUDA(cudaMemcpy(&num_rendered, geomState.point_offsets + P - 1, sizeof(u_int32_t), cudaMemcpyDeviceToHost),
               debug);
    std::cout << "here" << geomState.scan_size << std::endl;
