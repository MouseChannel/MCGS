                               ...
                               bool prefiltered)
{
    ...
    if ((rect_max.x - rect_min.x) * (rect_max.y - rect_min.y) == 0)
        return;
    // if (idx == 0) {
    //     printf("incuda%f %f  %d %d %d %d\n", p_proj.x, p_proj.y, rect_min.x, rect_min.y, rect_max.x, rect_max.y);
    // }
...
void FORWARD::preprocess(int P, int D, int M, const float* means3D, glm::vec3* scales, const float scale_modifier, const glm::vec4* rotations, const float* opacities, const float* shs, bool* clamped, const float* cov3D_precomp, const float* colors_precomp, const float* viewmatrix, const float* projmatrix, const glm::vec3* cam_pos, const int W, int H, const float focal_x, float focal_y, const float tan_fovx, float tan_fovy, int* radii, float2* means2D, float* depths, float* cov3Ds, float* rgb, float4* conic_opacity, const dim3 grid, uint32_t* tiles_touched, bool prefiltered)
{
    ...
    preprocessCUDA<NUM_CHANNELS><<<(P + 255) / 256, 256>>>(
        // preprocessCUDA<NUM_CHANNELS><<<1,1>>>(
        P,
        D,
        M,
        means3D,
