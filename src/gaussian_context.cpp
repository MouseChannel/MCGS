...
void GaussianContext::prepare()
{
    ...
    get_gaussian_raw_data();
    {
        ...
        m_out_img->SetImageLayout(vk::ImageLayout::eGeneral,
    ...
    gaussian_passes.emplace_back(std::make_shared<Process_Pass>(shared_from_this()));
    gaussian_passes.emplace_back(std::make_shared<sum_Pass>(shared_from_this()));
    gaussian_passes.emplace_back(std::make_shared<duplicate_Pass>(shared_from_this()));
    gaussian_passes.emplace_back(std::make_shared<MultiSortPass>(shared_from_this()));
    gaussian_passes.emplace_back(std::make_shared<Identify_Pass>(shared_from_this()));
    gaussian_passes.emplace_back(std::make_shared<Raster_Pass>(shared_from_this()));
}
std::shared_ptr<CommandBuffer> GaussianContext::BeginFrame()
{
    m_command_buffer->Begin(vk::CommandBufferUsageFlagBits::eOneTimeSubmit);
    for (auto& item : gaussian_passes) {
        item->Dispach(m_command_buffer->get_handle(), 0, 0, 0);
    }
    return m_command_buffer;
    //    precess_context->run_pass(m_command_buffer->get_handle());
    //    sum_context->run_pass(m_command_buffer->get_handle());
    //    duplicate_context->run_pass(m_command_buffer->get_handle());
    //    multi_sort_context->run_pass(m_command_buffer->get_handle());
    //    identify_content->run_pass(m_command_buffer->get_handle());
    //    render_content->run_pass(m_command_buffer->get_handle());
    //    camera->Tick();
}
void GaussianContext::Init()
{
    //    get_gaussian_raw_data();
...
void GaussianContext::Tick(vk::CommandBuffer cmd)
{
    for (auto& item : gaussian_passes) {
        item->Dispach(cmd, 0, 0, 0);
    }
}

GaussianContext::GeometryState::GeometryState(int size)
{
    depth_d.resize(size);
...
GaussianContext::BinningState::BinningState(int size)
{
...
GaussianContext::ImageState::ImageState(int size)
{
...
void GaussianContext::get_gaussian_raw_data()
{
    ...
    address_buffer = Buffer::CreateDeviceBuffer(&addr, sizeof(addr), vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddressKHR);
    // m_address.reset(new BufferDescriptorTarget(address_buffer,(int)Gaussian_Binding_Index::eAddress,vk::ShaderStageFlagBits::eCompute));

    // address = UniformManager::make_uniform({ addr }, vk::ShaderStageFlagBits::eCompute, vk::DescriptorType::eStorageBuffer);
}
void GaussianContext::Submit()
{
    ComputeContext::Submit();
}
void GaussianContext::EndFrame()
{
}
}