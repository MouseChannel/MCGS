#include "sort/sortMultiExcutePass.hpp"

namespace MCGS {
SortMulti_Pass::SortMulti_Pass(std::weak_ptr<GaussianContext> context)
    : ComputePass<GaussianContext>(context)
{
    Init();
}
void SortMulti_Pass::Dispach(vk::CommandBuffer cmd, int local_size_x, int local_size_y, int local_size_z)
{
...
{
    SetShaderModule("include/shaders/sort/multi_radixsort.comp.spv");
