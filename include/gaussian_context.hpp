...
class GaussianContext : public MCRT::ComputeContext, public std::enable_shared_from_this<GaussianContext> {
private:
    std::vector<std::shared_ptr<ComputePass<GaussianContext>>> gaussian_passes;
    //    ComputePass<>* a;
...
public:
    ...
    void Init();
    void Tick(vk::CommandBuffer cmd);
    // auto get_buffer_addr()
    // {
    //     return address;
    // }
    auto get_address_buffer()
    {
    ...
    std::shared_ptr<Image> m_out_img;
    //--
...
private:
    ...
    std::shared_ptr<Buffer> address_buffer;
    // std::shared_ptr<BufferDescriptorTarget> m_address;
